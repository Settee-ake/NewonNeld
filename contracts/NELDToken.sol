...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "Ownable Error: Caller is not the owner");
        _;
    }

    /**
     * @dev Leaves the contract without owner. It will not be possible to call any
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "Ownable Error: New owner is the same as current owner");
...
// Neld Token with Governance.
contract NeldToken is ERC20("Neld Token", "NELD"), Ownable {

  uint256 public constant CAP = 22000000e18; //22M
  uint256 public totalLock;


  uint256 public startReleaseBlock;
  uint256 public endReleaseBlock;
  uint256 public constant MANUAL_MINT_LIMIT = 1000000e18; //1M
  ...
  // Prevent excessive manual mint(s) that could break the cap limit
  ...
  function lock(address _account, uint256 _amount) external onlyOwner {
    ...
    require(endReleaseBlock > block.number, "no lock happen after endReleaseBlock has elapsed");
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When the block number is less than startReleaseBlock, no NELDs can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When the block number is more than endReleaseBlock, all locked NELDs can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some NELDs can be released
    else
    {
  ...
  function unlock() external {
    require(_locks[msg.sender] > 0, "no locked NELDs");

    uint256 amount = canUnlockAmount(msg.sender);

    require(amount != 0, "unlocked amount should not be ZERO");
  ...
  // @dev moves NELDs with its locked funds to another account
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "NELD::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "NELD::delegateBySig: invalid nonce");
    require(now <= expiry, "NELD::delegateBySig: signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
    require(blockNumber < block.number, "NELD::getPriorVotes: not yet determined");
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying NELDs (not scaled);
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "NELD::_writeCheckpoint: block number exceeds 32 bits");
